...
**keyATM** takes `matrix` or `data.frame` (`tibble`) type covariates data. If you have $D$ documents and $M$ covariates, the matrix should be $D \times M$. In this example, we have a dummy variable that indicates ...
...
out <- keyATM(
              ...
              options           = list(seed = 50, sampling_shift = 1, iterations = 10000)
             )
```
